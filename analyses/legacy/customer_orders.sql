-- This is the original customer orders SQL file before even basic source refactoring
-- The barely refactored code is at /models/legacy/customer_orders.sql
-- The fully refactored code is at /models/marts/fct_customer_orders.sql
...
from load.jaffle_shop.orders as orders

join (
      select 
        first_name || ' ' || last_name as name, 
        * 
      from load.jaffle_shop.customers
) customers
...
join (
    ...
    select 
        ...
        -- array_agg(distinct a.id) as order_ids    -- snowflake acceptable version, doesn't work in redshift
        listagg( distinct a.id ) as order_ids --redshift version  --why is this logic even here?

    from (
      select 
        row_number() over (partition by user_id order by order_date, id) as user_order_seq,
        *
      from load.jaffle_shop.orders
    ) a
    ...
    left outer join load.stripe.payment c
    on a.id = c.orderid
...
left outer join load.stripe.payment payments
